...
function signToken(user){
    ...
    const accessToken = jwt.sign(base, env.JWT_ACCESS_SECRET, { expiresIn: env.JWT_ACCESS_EXPIRED});
    const refreshToken = jwt.sign(base, env.JWT_REFRESH_SECRET, { expiresIn: env.JWT_REFRESH_EXPIRES });
...
exports.register = async (req, res) => {
    ...
    return res.status(201).json({ message: "Registered. Verification OTP sendto email. "})
}
...
exports.verifyEmail = async (req, res) => {
    ...
    const token = await OtpToken.findOne({ user: user._id, purpose: "verify", command: false}).sort({ createdAt: -1 });

    if(!token || token.expiresAt < new Date()) return res.status(400).json({ error: "OTP Expired ot not found"});
...
exports.loginWithPasswword = async (req, res) => {
    const { email, password } = req.body;
    const user = await User.findOne({ email });
...
exports.verifyLoginOtp = async (req, res) => {
    ...
    const token = await OtpToken.findOne({ user: user._id, purpose: "login", consumed: false}).sort({ createAt: -1});
